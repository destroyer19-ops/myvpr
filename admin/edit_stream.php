...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($title_err) && empty($description_err) && empty($stream_url_err)) {
        $sql = "UPDATE praise_live_tv SET title = ?, description = ?, stream_url = ?, is_live = ? WHERE id = ?";

        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("sssii", $title, $description, $stream_url, $is_live, $id);

            if ($stmt->execute()) {
                header("location: index.php");
            } else {
                echo "Oops! Something went wrong. Please try again later. Database error: " . $stmt->error;
                logDatabaseError($sql, $stmt->error);
            }
            $stmt->close();
        } else {
            echo "Oops! Something went wrong. Please try again later. Prepare failed: " . $conn->error;
    ...
    if ($id > 0) {
        $stmt = $conn->prepare("SELECT * FROM praise_live_tv WHERE id = ?");
